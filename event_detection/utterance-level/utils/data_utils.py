...
class DialogDataset(Dataset):
    def __init__(self, all_input_ids, all_token_type_ids, all_attention_mask, all_valid_mask, all_label_ids):
        # all_* : (num_data, num_dialog, max_seq_len)
    ...
    def __getitem__(self, index):
        dialog_input_ids = self.all_input_ids[index] # (num_dialog, max_seq_len)
...
def collate_fn(batch):
    ...
    batch_valid_mask = torch.stack(batch_valid_mask, dim=0)
...
def process_sentence(
    ...
    label_ids = [out_token] * len(sentence)
    if review:
    ...
    for example in examples:
        ...
        for index, sentence in enumerate(sentences):
            if index == 0:
                ...
                review = False
            input_ids, token_type_ids, attention_mask, valid_mask, label_ids = process_sentence(
                sentence,
                triggers,
                review,
                sent_start,
