...
class BertModel(torch.nn.Module):
    ...
    def forward(self, batch_input_ids, batch_token_type_ids, batch_attention_mask, batch_valid_mask, batch_label_ids, mode):
        ...
        for i in range(batch_size):
            sent_id = 0 # int
            valid_mask = batch_valid_mask[i] # (max_seq_len)
            label_ids = batch_label_ids[i]
            attention_mask = batch_attention_mask[i][sent_id]
            dialog_input_ids = batch_input_ids[i] # (num_dialog, max_seq_len)
            ...
            last_hidden_state = outputs['last_hidden_state'] # (num_dialog, max_seq_len, hidden_size)
            ...
            sent_state = last_hidden_state[sent_id] # (max_seq_len, hidden_size)
            dialog_state_ = dialog_state.repeat(sent_state.size(0), 1)
            sent_state_ = torch.cat((sent_state, dialog_state_), dim=-1) # (max_seq_len, hidden_size*2)
            gate_weight = self.sigmoid(self.W_2(sent_state_)) # (max_seq_len, 1)
            gate_weight_ = torch.ones_like(gate_weight) - gate_weight

            gate_state = torch.mul(gate_weight, sent_state) + torch.mul(gate_weight_, dialog_state_)
            new_sent_state = torch.cat((sent_state, gate_state), dim=-1) # (max_seq_len, hidden_size*2)
